import re
from models import AddressBook, Record, Email
...
@input_error_decorator_factory(args_length=3, message="Invalid command. Usage: add_email <name> <email>")
def add_email(name: str, email: str, book: AddressBook) -> None:
    record = book.find(name)
    record.add_email(email)
    print(f"Email added for {name}: {email}")

@input_error_decorator_factory(args_length=3, message="Invalid command. Usage: edit_email <name> <new_email>")
def edit_email(name: str, new_email: str, book: AddressBook) -> None:
    record = book.find(name)
    record.edit_email(new_email)
    print(f"Email updated for {name}: {new_email}")

@input_error_decorator_factory(args_length=2, message="Invalid command. Usage: remove_email <name>")
